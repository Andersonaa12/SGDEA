        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col">
                ...
                <nav class="flex text-sm text-gray-600 mt-1">
                    ...
                    <span class="text-indigo-600 font-medium">Editar {{ $user->name }}</span>
                </nav>
            </div>
        </div>
    </x-slot>
                ...
                <div class="p-8">
                    <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="identification" id="identification" value="{{ old('identification', $user->identification) }}" required
                                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="active" {{ old('active', $user->active) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-indigo-600">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        @foreach ($roles as $role)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="roles[]" value="{{ $role->id }}" {{ $user->hasRole($role->name) || in_array($role->id, old('roles', [])) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-indigo-600">
                                ...
                                <div>
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        @foreach ($permissions as $permission)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" {{ $user->hasDirectPermission($permission->name) || in_array($permission->name, old('permissions', [])) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-indigo-600">
                        ...
                        <div class="mt-8">
                            ...
                            <div x-data="{ sections: @json($user->sections->map(fn($s) => ['section_id' => $s->id, 'role_in_section' => $s->pivot->role_in_section])) }" class="space-y-4">
                                    ...
                                    <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-lg">
                                        <select :name="'sections[' + index + '][section_id]'" required
                                            ...
                                            @foreach ($sections as $sec)
                                                <option value="{{ $sec->id }}" x-bind:selected="section.section_id == {{ $sec->id }}">{{ $sec->name }}</option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="mt-8">
                            ...
                            <div x-data="{ subsections: @json($user->subsections->map(fn($s) => ['subsection_id' => $s->id, 'role_in_subsection' => $s->pivot->role_in_subsection])) }" class="space-y-4">
                                    ...
                                    <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-lg">
                                        <select :name="'subsections[' + index + '][subsection_id]'" required
                                            ...
                                            @foreach ($subsections as $sub)
                                                <option value="{{ $sub->id }}" x-bind:selected="subsection.subsection_id == {{ $sub->id }}">{{ $sub->name }}</option>
                                            @endforeach
                                        </select>
