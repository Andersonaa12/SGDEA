        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col">
                ...
                <nav class="flex text-sm text-gray-600 mt-1">
                    ...
                    <span class="text-indigo-600 font-medium">Editar {{ $role->name }}</span>
                </nav>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-2">
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8 py-4">
            <div class="bg-white shadow-xl rounded-lg overflow-hidden">
                <div class="p-8">
                    <form action="{{ route('users.roles.update', $role) }}" method="POST" class="space-y-8">
                        @csrf
                        @method('PUT')
                                    ...
                                    <div class="grid grid-cols-2 gap-2">
                                        @foreach ($permissions as $permission)
                                            <label class="inline-flex items-center">
                                                <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" {{ $role->hasPermissionTo($permission->name) || in_array($permission->name, old('permissions', [])) ? 'checked' : '' }} class="form-checkbox h-5 w-5 text-indigo-600">
                        ...
                        <div class="flex gap-4 pt-8 border-t">
                            ...
                            <a href="{{ route('users.roles.show', $role) }}"
