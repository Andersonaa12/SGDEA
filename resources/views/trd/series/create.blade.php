        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex flex-col">
                ...
                <nav class="flex text-sm text-gray-600 mt-1">
                    ...
                    <span class="text-indigo-600 font-medium">Crear Serie</span>
                </nav>
            </div>
        </div>
    </x-slot>
                    ...
                    <form action="{{ route('series.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="trd_id" id="trd_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                ...
                                @foreach ($trds as $trd)
                                    <option value="{{ $trd->id }}" {{ old('trd_id') == $trd->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                      placeholder="Descripción general de los documentos que agrupa esta serie...">{{ old('description') }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="retention_management_years" id="retention_management_years"
                                       value="{{ old('retention_management_years') }}" min="0" max="99"
                            ...
                            <div>
                                ...
                                <input type="number" name="retention_central_years" id="retention_central_years"
                                       value="{{ old('retention_central_years') }}" min="0" max="99"
                                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                        ...
                        <div>
                            ...
                            <div class="space-y-3">
                                ...
                                @foreach($options as $value => $opt)
                                    <label class="flex items-start p-4 border rounded-lg cursor-pointer hover:bg-gray-50 transition {{ old('final_disposition') == $value ? 'ring-2 ring-indigo-500 border-indigo-500' : '' }}">
                                        <input type="radio" name="final_disposition" value="{{ $value }}"
                                               {{ old('final_disposition') == $value ? 'checked' : '' }} required class="mt-1">
                                        <div class="ml-4">
                        ...
                        <div>
                            ...
                            <textarea name="disposition_procedure" id="disposition_procedure" rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                      placeholder="Ej: Acta de eliminación firmada por comité archivístico...">{{ old('disposition_procedure') }}</textarea>
